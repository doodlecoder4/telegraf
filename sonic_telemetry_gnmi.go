	...
	if value != nil {
		...
		if reflect.TypeOf(value).Kind() == reflect.String {
			if strings.Contains(value, ",") {
				...
				for _, val := range split_value[0:] {
					if n, err := strconv.ParseFloat(val, 64); err != nil {
						c.acc.AddError(fmt.Errorf("Failed to Split String to float: %value", err))
					} else {
						float_values = append(float_values, n)
					}
				}
